"""Profil sahifasi — chuqur statistik tahlil, sifat tekshiruvi, bog'liqliklar."""
...
class ProfilePage(BasePage):
    title = "Profil va tahlil"
    subtitle = ("Har bir ustun bo'yicha to'liq statistika, ma'lumot sifati bo'yicha "
                "avtomatik ogohlantirishlar va bog'liqlik tahlili.")
    ...
    def _build(self) -> None:
        ...
        for key, label in (("rows", "Qatorlar"), ("cols", "Ustunlar"),
                           ("memory", "Xotira"), ("missing", "Bo'sh qiymat"),
                           ("dupes", "Dublikat"), ("quality", "Sifat bahosi")):
            tile = StatTile(label)
            self.tiles[key] = tile
        ...
        row = QHBoxLayout()
        self.run_btn = make_button("Profilni hisoblash", "Primary", self._run_profile)
        ...
        self.tabs.addTab(self._tab_columns(), "Ustunlar")
        self.tabs.addTab(self._tab_issues(), "Sifat muammolari")
        self.tabs.addTab(self._tab_corr(), "Bog'liqliklar")
        self.tabs.addTab(self._tab_group(), "Guruhlash / Pivot")
        self.tabs.addTab(self._tab_time(), "Vaqt tahlili")
    ...
    def _tab_columns(self) -> QWidget:
        ...
        hint = QLabel("Ustun qatoriga bosib pastda uning taqsimotini ko'ring.")
    ...
    def _tab_corr(self) -> QWidget:
        ...
        row.addWidget(make_button("Kategorik bog'liqlik (Cramér's V)", "Ghost",
                                  self._draw_cramers))
        row.addStretch(1)
        lay.addLayout(row)
    ...
    def _tab_time(self) -> QWidget:
        ...
        row.addWidget(QLabel("Chastota:"))
        self.time_freq = QComboBox()
        self.time_freq.addItems(["avtomatik", "1s", "1min", "5min", "15min",
                                 "1h", "6h", "1D", "1W", "1ME"])
    ...
    def refresh(self) -> None:
        ...
        self.status.setText("Ma'lumot o'zgardi — profilni qayta hisoblang."
    ...
    def _on_profile(self, p) -> None:
        self._profile = p
        self.run_btn.setEnabled(True)
        self.status.setText("✓ Profil tayyor")
        ...
        if not p.issues:
            self.issues_list.addItem("✓ Jiddiy muammo topilmadi.")
        for iss in p.issues:
            color = {"yuqori": PALETTE["danger"], "o'rta": PALETTE["warning"]}.get(
                iss.get("level", "past"), PALETTE["faint"])
            item = QListWidgetItem(
                f"[{iss.get('level', 'past').upper()}]  {iss['title']}  —  "
                f"{iss.get('column', '')}\n        {iss['detail']}")
    ...
    def _on_column_click(self, index) -> None:
        if self._profile is None:
            return
        try:
            name = str(self._profile.to_frame().iloc[index.row()]["ustun"])
        except Exception:
            return
        df = self.df
        ...
        try:
            ...
            self.col_chart.show_message(f"Grafik chizilmadi: {exc}")
    ...
    def _draw_corr(self) -> None:
        ...
        try:
            ...
            if corr.empty:
                self.corr_canvas.show_message("Kamida 2 ta sonli ustun kerak")
            ...
            self.corr_canvas.show_message(f"Xato: {exc}")
    ...
    def _draw_cramers(self) -> None:
        ...
        try:
            ...
            self.notify(f"Hisoblanmadi: {exc}", "error")
            return
        if m.empty:
            self.notify("Kamida 2 ta kategorik ustun kerak", "warn")
            return
        self.corr_table.set_df(m.reset_index().rename(columns={"index": "ustun"}))
        self.notify("Cramér's V matritsasi jadvalda ko'rsatildi", "success")

    def _run_group(self) -> None:
        by = self.group_by.values()
        if not by:
            self.notify("Guruhlash ustunini tanlang", "warn")
            return
    ...
    def _group_to_dataset(self) -> None:
        ...
        if res is None or res.empty:
            self.notify("Avval guruhlashni hisoblang", "warn")
            return
        self.store.add(f"{self.store.active_name}_guruh", res)
