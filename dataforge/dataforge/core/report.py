"""HTML hisobot generatori — profil, grafiklar va ML natijalarini bitta faylga jamlaydi."""
...
.issue.yuqori {{ border-left-color: {PALETTE['danger']}; }}
.issue.past {{ border-left-color: {PALETTE['faint']}; }}
...
def _table(df: pd.DataFrame, max_rows: int = 200) -> str:
    if df is None or df.empty:
        return f"<p style='color:{PALETTE['muted']}'>Ma'lumot yo'q</p>"
    ...
    more = (f"<p style='color:{PALETTE['muted']};font-size:12px'>"
            f"… jami {len(df):,} qatordan {max_rows} tasi ko'rsatildi</p>"
...
def build_report(
    df: pd.DataFrame,
    *,
    title: str = "Ma'lumot tahlili hisoboti",
    ...
    """To'liq HTML hisobot matnini quradi."""
    ...
    parts.append(
        f"<div class='sub'>Manba: <b>{html.escape(dataset_name)}</b> · "
        f"Yaratildi: {now} · {APP_NAME} v{VERSION}</div>"
    )

    # Umumiy ko'rsatkichlar
    parts.append("<div class='cards'>")
    parts.append(_card("Qatorlar", f"{prof.rows:,}"))
    parts.append(_card("Ustunlar", f"{prof.cols:,}"))
    parts.append(_card("Xotira", f"{prof.memory_mb:.2f} MB"))
    parts.append(_card("Bo'sh qiymat", f"{prof.missing_pct:.2f}%"))
    parts.append(_card("Dublikatlar", f"{prof.duplicates:,}"))
    parts.append(_card("Sifat bahosi", f"{prof.quality_score}/100"))
    parts.append("</div>")

    # Ustunlar profili
    parts.append("<h2>Ustunlar profili</h2>")
    parts.append(_table(prof.to_frame(), 300))

    # Sifat muammolari
    if prof.issues:
        parts.append("<h2>Ma'lumot sifati bo'yicha ogohlantirishlar</h2>")
        for iss in prof.issues[:60]:
            parts.append(
                f"<div class='issue {html.escape(iss.get('level', 'past'))}'>"
    ...
    # Tahrirlar tarixi
    if history:
        parts.append("<h2>Qo'llangan amallar</h2>")
        parts.append("<pre>" + html.escape("\n".join(
            f"{i + 1:2d}. {h}" for i, h in enumerate(history))) + "</pre>")

    # Namuna
    parts.append("<h2>Ma'lumot namunasi</h2>")
    parts.append(_table(df.head(50), 50))

    parts.append(f"<footer>{APP_NAME} v{VERSION} tomonidan yaratildi · {now}</footer>")
