"""Loyiha (sessiya) saqlash va yuklash — .dfp fayli (ZIP ichida parquet + meta)."""
...
def save_project(path: str | Path, datasets: dict[str, pd.DataFrame],
                 ...
                 notes: str = "") -> str:
    """Barcha jadvallarni bitta loyiha fayliga saqlaydi."""
    p = Path(path)
...
def _to_parquet_bytes(df: pd.DataFrame) -> bytes:
    ...
    buf = io.BytesIO()
    safe = df.copy()
    # parquet aralash turlarni qabul qilmaydi — obyekt ustunlarni matnga keltiramiz
...
def load_project(path: str | Path) -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    """Loyiha faylini o'qib, jadvallar va metama'lumotni qaytaradi."""
    import io
...
def project_info(path: str | Path) -> dict[str, Any]:
    """Faylni to'liq yuklamasdan qisqacha ma'lumot oladi."""
