"""Grafik qurish yadrosi — matplotlib figuralari (GUI va hisobot uchun umumiy).

``CHART_TYPES`` — qo'llab-quvvatlanadigan grafik turlari va ular talab qiladigan
maydonlar. GUI shu ta'rifga qarab forma quradi.
"""
...
matplotlib.use("Agg")  # GUI kanvasi o'z backend'ini o'rnatadi
...
def apply_dark_style() -> None:
    """Matplotlib uchun dastur temasiga mos qorong'i uslub."""
    plt.rcParams.update({
...
class ChartSpec:
    ...
    label: str
    group: str
    needs: list[str] = field(default_factory=list)      # majburiy maydonlar
    optional: list[str] = field(default_factory=list)   # ixtiyoriy maydonlar
    x_kind: str = "any"     # any | num | cat | dt
    y_kind: str = "any"


CHART_TYPES: dict[str, ChartSpec] = {
    "line":       ChartSpec("line", "Chiziqli", "Asosiy", ["x", "y"], ["hue"]),
    "area":       ChartSpec("area", "Maydon (area)", "Asosiy", ["x", "y"], ["hue"]),
    "step":       ChartSpec("step", "Pog'onali", "Asosiy", ["x", "y"], ["hue"]),
    "bar":        ChartSpec("bar", "Ustunli", "Asosiy", ["x"], ["y", "hue"], x_kind="cat"),
    "barh":       ChartSpec("barh", "Yotiq ustunli", "Asosiy", ["x"], ["y", "hue"], x_kind="cat"),
    "stacked_bar": ChartSpec("stacked_bar", "Yig'ma ustunli", "Asosiy", ["x", "hue"], ["y"],
                             x_kind="cat"),
    "pie":        ChartSpec("pie", "Doiraviy", "Asosiy", ["x"], ["y"], x_kind="cat"),
    "donut":      ChartSpec("donut", "Halqa (donut)", "Asosiy", ["x"], ["y"], x_kind="cat"),
    "count":      ChartSpec("count", "Chastota (count)", "Asosiy", ["x"], ["hue"], x_kind="cat"),

    "scatter":    ChartSpec("scatter", "Nuqtali", "Taqsimot", ["x", "y"], ["hue", "size"],
                            x_kind="num", y_kind="num"),
    "bubble":     ChartSpec("bubble", "Pufakchali", "Taqsimot", ["x", "y", "size"], ["hue"],
                            x_kind="num", y_kind="num"),
    "hexbin":     ChartSpec("hexbin", "Hexbin zichlik", "Taqsimot", ["x", "y"], [],
                            x_kind="num", y_kind="num"),
    "hist":       ChartSpec("hist", "Gistogramma", "Taqsimot", ["x"], ["hue"], x_kind="num"),
    "kde":        ChartSpec("kde", "Zichlik (KDE)", "Taqsimot", ["x"], ["hue"], x_kind="num"),
    "ecdf":       ChartSpec("ecdf", "ECDF", "Taqsimot", ["x"], ["hue"], x_kind="num"),
    "box":        ChartSpec("box", "Quti (box)", "Taqsimot", ["y"], ["x", "hue"], y_kind="num"),
    "violin":     ChartSpec("violin", "Skripka (violin)", "Taqsimot", ["y"], ["x"], y_kind="num"),
    "strip":      ChartSpec("strip", "Nuqtali tarqoqlik", "Taqsimot", ["y"], ["x"], y_kind="num"),

    "heatmap":    ChartSpec("heatmap", "Issiqlik xaritasi", "Bog'liqlik", ["x", "y"], ["value"]),
    "corr":       ChartSpec("corr", "Korrelyatsiya matritsasi", "Bog'liqlik", [], []),
    "pairplot":   ChartSpec("pairplot", "Juftliklar matritsasi", "Bog'liqlik", [], ["hue"]),
    "missing":    ChartSpec("missing", "Bo'sh qiymatlar xaritasi", "Bog'liqlik", [], []),

    "timeseries": ChartSpec("timeseries", "Vaqt qatori", "Vaqt", ["x"], ["y", "hue"], x_kind="dt"),
    "calendar":   ChartSpec("calendar", "Soat × hafta kuni", "Vaqt", ["x"], ["y"], x_kind="dt"),
    "rolling":    ChartSpec("rolling", "Siljuvchi o'rtacha", "Vaqt", ["x", "y"], [], x_kind="dt"),
}
...
def _limit_categories(s: pd.Series, top: int = MAX_CATEGORIES) -> pd.Series:
    ...
    return s.where(s.isin(keep), other="(boshqa)")
...
def _agg_series(df: pd.DataFrame, x: str, y: str | None, agg: str) -> pd.DataFrame:
    """x bo'yicha guruhlab y ni jamlaydi."""
    if y is None or agg == "count":
        out = df.groupby(x, dropna=False, observed=False).size().reset_index(name="qiymat")
    else:
        g = df.groupby(x, dropna=False, observed=False)[y]
        out = getattr(g, agg)().reset_index().rename(columns={y: "qiymat"})
    return out
...
def build_chart(df: pd.DataFrame, kind: str, x: str | None = None, y: str | None = None,
    ...
    """Berilgan tur bo'yicha matplotlib figurasi quradi."""
    if df is None or df.empty:
        return _empty_figure("Ma'lumot yo'q", figsize, dpi)

    spec = CHART_TYPES.get(kind)
    if spec is None:
        raise ValueError(f"Noma'lum grafik turi: {kind}")
    for need in spec.needs:
        if not locals().get(need) and not opts.get(need):
            raise ValueError(f"'{spec.label}' uchun '{need}' maydoni kerak")

    fn = _BUILDERS.get(kind)
    if fn is None:
        raise ValueError(f"'{kind}' uchun quruvchi topilmadi")
...
def _b_bar(fig, df, x, y, hue, size, agg, opts, horizontal=False):
    ...
    if hue:
        ...
        s = s.sort_values("qiymat", ascending=horizontal)
        color = CHART_COLORS[0]
        if horizontal:
            ax.barh([str(v) for v in s[x]], s["qiymat"], color=color)
        else:
            ax.bar([str(v) for v in s[x]], s["qiymat"], color=color)
            opts.setdefault("rotate_x", len(s) > 6)
        if opts.get("labels", True) and len(s) <= 25:
            for i, v in enumerate(s["qiymat"]):
                if horizontal:
...
def _b_stacked_bar(fig, df, x, y, hue, size, agg, opts):
    ...
    ax.set_ylabel("%" if opts.get("percent") else (y or "soni"))
...
def _b_pie(fig, df, x, y, hue, size, agg, opts, donut=False):
    ax = fig.add_subplot(111)
    s = _agg_series(df, x, y, agg).sort_values("qiymat", ascending=False)
    top = int(opts.get("top", 10))
    if len(s) > top:
        head = s.head(top).copy()
        rest = pd.DataFrame({x: ["(boshqa)"], "qiymat": [s["qiymat"][top:].sum()]})
        s = pd.concat([head, rest], ignore_index=True)
    wedges, _, autotexts = ax.pie(
        s["qiymat"], labels=[str(v) for v in s[x]], autopct="%1.1f%%",
    ...
    for t in autotexts:
        ...
        t.set_fontsize(8)
    if donut:
        ax.text(0, 0, f"{s['qiymat'].sum():,.0f}", ha="center", va="center",
...
def _b_hexbin(fig, df, x, y, hue, size, agg, opts):
    ...
    cb.set_label("soni", color=PALETTE["muted"])
    cb.ax.tick_params(colors=PALETTE["muted"])
    ax.set_ylabel(y)
    return ax
...
def _b_hist(fig, df, x, y, hue, size, agg, opts):
    ...
    if hue:
        ...
        if opts.get("stats", True) and not vals.empty:
            for val, color, label in ((vals.mean(), PALETTE["warning"], "o'rtacha"),
                                      (vals.median(), PALETTE["success"], "mediana")):
...
def _b_box(fig, df, x, y, hue, size, agg, opts, violin=False, strip=False):
    ...
    if x:
        ...
        cats = [y]
    if not data:
        ax.text(0.5, 0.5, "Ma'lumot yo'q", ha="center", color=PALETTE["muted"])
        return ax
...
def _b_corr(fig, df, x, y, hue, size, agg, opts):
    ...
    if corr.empty:
        ax.text(0.5, 0.5, "Kamida 2 ta sonli ustun kerak", ha="center",
                color=PALETTE["muted"])
        ax.set_axis_off()
        return ax
...
def _b_pairplot(fig, df, x, y, hue, size, agg, opts):
    ...
    for i in range(n):
        for j in range(n):
            ...
            ax.grid(alpha=0.25)
    fig.suptitle("Juftliklar matritsasi", color=PALETTE["text"])
    try:
        fig.tight_layout()
    except Exception:
        pass
...
def _b_missing(fig, df, x, y, hue, size, agg, opts):
    ...
    ax.set_xlabel("qator (namuna)")
    ax.grid(False)
    pct = 100 * df.isna().sum().sum() / max(1, df.size)
    ax.set_title(f"Bo'sh qiymatlar xaritasi — jami {pct:.2f}%")
    return ax
...
def _b_timeseries(fig, df, x, y, hue, size, agg, opts):
    ...
    ax.set_ylabel(f"{agg}({y})" if y else "hodisalar soni")
...
def _b_rolling(fig, df, x, y, hue, size, agg, opts):
    ...
    if ts.empty:
        ...
        return ax
    vals = ts.iloc[:, 1]
    ax.plot(ts.iloc[:, 0], vals, color=PALETTE["faint"], linewidth=1.0, label="xom")
...
def _b_calendar(fig, df, x, y, hue, size, agg, opts):
    ...
    if d.empty:
        ax.text(0.5, 0.5, "Vaqt ma'lumoti yo'q", ha="center", color=PALETTE["muted"])
        return ax
    try:
        d = d.dt.tz_localize(None)
    except (TypeError, AttributeError):
        pass
    tmp = pd.DataFrame({"soat": d.dt.hour, "kun": d.dt.dayofweek})
    if y and y in df.columns:
        tmp["val"] = pd.to_numeric(df.loc[d.index, y], errors="coerce")
        pivot = tmp.pivot_table(index="kun", columns="soat", values="val",
                                aggfunc=agg if agg != "count" else "mean")
    else:
        pivot = tmp.pivot_table(index="kun", columns="soat", aggfunc="size")
    pivot = pivot.reindex(index=range(7), columns=range(24)).fillna(0)
    im = ax.imshow(pivot.to_numpy(), cmap="viridis", aspect="auto")
    ax.set_yticks(range(7), ["Du", "Se", "Ch", "Pa", "Ju", "Sh", "Ya"])
...
# ---------------------------------------------------------------------------
# ML natijalari uchun maxsus grafiklar
# ---------------------------------------------------------------------------
def confusion_figure(cm: pd.DataFrame, normalize: bool = False,
    ...
    ax.set_xticks(range(cm.shape[1]), [str(c).replace("bashorat ", "") for c in cm.columns],
                  rotation=45, ha="right")
    ax.set_yticks(range(cm.shape[0]), [str(i).replace("haqiqiy ", "") for i in cm.index])
    ax.set_xlabel("Bashorat")
    ax.set_ylabel("Haqiqiy")
    ax.set_title("Chalkashlik matritsasi" + (" (%)" if normalize else ""))
    vmax = data.max() or 1
...
def roc_figure(curves: dict[str, tuple], figsize=(6.5, 5.5)) -> Figure:
    ...
    ax.set_xlabel("FPR")
    ax.set_ylabel("TPR")
    ax.set_title("ROC egri chizig'i")
...
def importance_figure(imp: pd.DataFrame, top: int = 20, figsize=(7.5, 5.5)) -> Figure:
    ...
    ax.barh(d["xususiyat"].astype(str), d["muhimlik"], color=CHART_COLORS[0])
    ax.set_title("Xususiyatlar muhimligi")
    ax.set_xlabel("muhimlik")
...
def residual_figure(y_true: np.ndarray, y_pred: np.ndarray, figsize=(7.5, 5.5)) -> Figure:
    ...
    axes[0].set_title("Haqiqiy vs Bashorat")
    ...
    axes[1].set_ylabel("Qoldiq")
    axes[1].set_title("Qoldiqlar taqsimoti")
    for ax in axes:
        ax.grid(alpha=0.4)
    fig.tight_layout()
    return fig
...
def cluster_figure(coords: np.ndarray, labels: np.ndarray, title: str = "Klasterlar",
    ...
    for lab, color in zip(uniq, cols):
        m = labels == lab
        name = "shovqin" if lab == -1 else f"klaster {lab}"
...
def anomaly_figure(values: pd.Series, flags: np.ndarray, scores: np.ndarray | None = None,
    ...
    ax.plot(xs, values, color=CHART_COLORS[0], linewidth=1.2, label="qiymat", alpha=0.9)
    if flags.any():
        ax.scatter(xs[flags], values[flags], color=PALETTE["danger"], s=42, zorder=5,
                   label=f"anomaliya ({int(flags.sum()):,})", edgecolors="none")
    ax.legend(frameon=True)
    ax.set_title("Anomaliyalar")
    ax.grid(alpha=0.4)
    del scores
    fig.tight_layout()
    return fig
...
def forecast_figure(history: pd.Series, fc: pd.DataFrame, figsize=(9.5, 5.4)) -> Figure:
    ...
    ax = fig.add_subplot(111)
    ax.set_facecolor(PALETTE["surface"])
    ax.plot(history.index, history.to_numpy(), color=CHART_COLORS[0], label="tarix")
    ax.plot(fc["vaqt"], fc["bashorat"], color=PALETTE["warning"], linestyle="--",
            label="bashorat")
    ax.fill_between(fc["vaqt"], fc["past"], fc["yuqori"], color=PALETTE["warning"],
                    alpha=0.18, label="95% ishonch")
    ax.legend(frameon=True)
    ax.set_title("Vaqt qatori bashorati")
    ax.grid(alpha=0.4)
    for lbl in ax.get_xticklabels():
        lbl.set_rotation(30)
        lbl.set_ha("right")
...
def elbow_figure(scores: pd.DataFrame, figsize=(7.0, 4.6)) -> Figure:
    ...
    ax = fig.add_subplot(111)
    ax.set_facecolor(PALETTE["surface"])
    ax.plot(scores["k"], scores["siluet"], marker="o", color=CHART_COLORS[0],
            label="siluet")
    best = scores.loc[scores["siluet"].idxmax()]
    ax.axvline(best["k"], color=PALETTE["success"], linestyle="--",
               label=f"optimal k={int(best['k'])}")
    ax.set_xlabel("klasterlar soni (k)")
    ax.set_ylabel("siluet bahosi")
    ax.set_title("Optimal k tanlash")
    ax.legend(frameon=True)
    ax.grid(alpha=0.4)
    fig.tight_layout()
    return fig
...
def figure_to_base64(fig: Figure, fmt: str = "png", dpi: int = 120) -> str:
    """Figurani HTML hisobotga joylash uchun base64 satrga aylantiradi."""
    buf = io.BytesIO()
