...
def window(app, context):
    ...
    app.processEvents()
    yield main
    main._sync.stop()   # noqa: SLF001 — testda fon oqimini to'xtatamiz
    main.hide()
...
def test_every_navigation_entry_has_a_page(window) -> None:
    ...
    missing = [key for key in ALL_PAGE_KEYS if key not in window._pages]  # noqa: SLF001
...
def test_page_opens_and_refreshes(window, app, key: str) -> None:
    ...
    assert window._stack.currentWidget() is page   # noqa: SLF001
...
def test_inventory_shows_computed_stock(window, app) -> None:
    window._select("inventory")   # noqa: SLF001
    app.processEvents()
    page = window._pages["inventory"]   # noqa: SLF001
...
def test_sync_page_reports_device_state(window, app) -> None:
    window._select("sync")   # noqa: SLF001
    app.processEvents()
    page = window._pages["sync"]   # noqa: SLF001
    assert "yozuv" in page.header._subtitle.text()   # noqa: SLF001
...
def test_reports_page_builds_every_report(window, app) -> None:
    ...
    window._select("reports")   # noqa: SLF001
    app.processEvents()
    page = window._pages["reports"]   # noqa: SLF001

    for index in range(page._choice.count()):   # noqa: SLF001
        page._choice.setCurrentIndex(index)   # noqa: SLF001
        page._on_build()   # noqa: SLF001
        app.processEvents()
        assert page._report is not None, (   # noqa: SLF001
            f"hisobot shakllanmadi: {page._choice.itemText(index)}"   # noqa: SLF001
        )


def test_assistant_produces_suggestions(window, app) -> None:
    """Namuna ma'lumotda qoldiq minimaldan past — tavsiya chiqishi kerak."""
    window._select("assistant")   # noqa: SLF001
    app.processEvents()
    page = window._pages["assistant"]   # noqa: SLF001
    assert page.table.row_count() > 0, "AI hech qanday tavsiya bermadi"


def test_security_page_shows_protocol(window, app) -> None:
    window._select("security")   # noqa: SLF001
    app.processEvents()
    page = window._pages["security"]   # noqa: SLF001
    assert "AETHER-Q" in page._card_protocol._value.text()   # noqa: SLF001


def test_settings_page_hides_no_secrets(window, app) -> None:
    """Diagnostika matnida kalit yoki maxfiy material BO'LMASLIGI kerak."""
    window._select("settings")   # noqa: SLF001
    app.processEvents()
    page = window._pages["settings"]   # noqa: SLF001
    text = page._info.toPlainText()   # noqa: SLF001
