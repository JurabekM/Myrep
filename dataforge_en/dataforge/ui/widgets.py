...
class PageHeader(QWidget):
    """A page title with a description."""
...
class DataFrameModel(QAbstractTableModel):
    """Binds a DataFrame to a QTableView with editing support."""

    edit_requested = Signal(int, str, object)   # (row, column, new value)
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if role in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.ToolTipRole):
            if value is None or (not isinstance(value, (list, dict, np.ndarray))
                                 and pd.isna(value)):
                return "" if role == Qt.ItemDataRole.DisplayRole else "missing (NaN)"
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            ...
            if colname.lower() in ("level", "severity", "state", "status"):
...
class FrameTable(QTableView):
    """A table view tuned for displaying DataFrames."""

    column_action = Signal(str, str)   # (action, column)
...
KIND_FILTERS = {
    ...
    "cat": lambda s: column_kind(s) in (CATEGORICAL, "boolean"),
    "dt": lambda s: column_kind(s) == DATETIME,
    "text": lambda s: column_kind(s) in ("text", CATEGORICAL),
}


class ColumnCombo(QComboBox):
    """A drop-down that lists columns, filtered by kind."""
...
class MultiColumnSelect(QWidget):
    """Multi-column picker with a search box and select-all buttons."""
...
class ChartCanvas(QWidget):
    """A widget that renders a matplotlib figure with a navigation toolbar."""
    ...
    def __init__(self, parent: QWidget | None = None, toolbar: bool = True) -> None:
        ...
        self.show_message("Choose the settings to build a chart")

    def set_figure(self, fig) -> None:
        """Display a new figure, replacing the previous one."""
        lay = self.layout()
        old = self.canvas
        self.figure = fig
...
class ScrollPage(QScrollArea):
    """A vertically scrollable page container."""
    ...
    def __init__(self, parent: QWidget | None = None, margins=(20, 18, 20, 20),
        ...
        inner = QWidget()
        # The style must apply to this container only — otherwise it makes the
        # background of the buttons inside it transparent as well.
...
class WheelGuard(QObject):
    """Stop unfocused combo boxes and spin boxes from reacting to the wheel.

    Prevents scrolling a panel from accidentally changing the value of whatever
    drop-down happens to sit under the mouse pointer.
    """
