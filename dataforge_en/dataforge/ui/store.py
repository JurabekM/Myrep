"""Central data store — every page talks to the app through this object."""
...
class DataStore(QObject):
    """Owns the loaded tables, the active table and the undo/redo history."""

    datasets_changed = Signal()            # the list changed
    active_changed = Signal(str)           # the active table switched
    data_changed = Signal()                # the active table's content changed
    message = Signal(str, str)             # (text, level: info|success|warn|error)
    ...
    def df(self) -> pd.DataFrame:
        """The active table (an empty DataFrame when nothing is loaded)."""
    ...
    def add(self, name: str, df: pd.DataFrame, meta: dict[str, Any] | None = None,
        ...
        """Add a new table (a counter is appended when the name is taken)."""
        ...
        self._hist[name] = History(df, label=f"Loaded · {df.shape[0]:,}×{df.shape[1]}")
        self.datasets_changed.emit()
        if activate:
            self.set_active(name)
        self.message.emit(f"'{name}' loaded: {len(df):,} rows × {df.shape[1]} columns",
                          "success")
        return name
    ...
    def commit(self, df: pd.DataFrame, label: str, name: str | None = None) -> None:
        """Push a new state onto the history and update the active table."""
    ...
    def run_op(self, key: str, **params: Any) -> bool:
        """Apply a registered transform operation to the active table."""
        df = self.df()
        if df.empty and key not in ("reset_index",):
            self.message.emit("Load some data first", "warn")
            return False
        try:
            new = apply_op(df, key, **params)
        except Exception as exc:
            self.message.emit(f"Operation failed: {exc}", "error")
            return False
        self.commit(new, describe_op(key, params))
        return True

    def set_cell(self, row: int, col: str, value: Any) -> bool:
        """Edit a single cell (recorded in the history)."""
        df = self.df()
        ...
        try:
            ...
            if pd.api.types.is_numeric_dtype(series) and value != "":
                ...
                cast = str(value).strip().lower() in ("1", "true", "yes")
            else:
                cast = value
            ...
            self.message.emit(f"Could not edit the cell: {exc}", "error")
            return False
        self.commit(new, f"Cell edited [{row}, {col}]")
        return True
    ...
    def add_row(self, at: int | None = None) -> None:
        ...
        self.commit(new, "Row added")

    def add_column(self, name: str, default: Any = None) -> None:
        df = self.df().copy()
        if not name or name in df.columns:
            self.message.emit("The column name is empty or already used", "warn")
            return
        df[name] = default
        self.commit(df, f"Column added: {name}")
    ...
    def undo(self) -> None:
        ...
        if hist and hist.can_undo:
            ...
            self.data_changed.emit()
            self.message.emit(f"Undone: {label}", "info")
    ...
    def redo(self) -> None:
        ...
        if hist and hist.can_redo:
            ...
            self.data_changed.emit()
            self.message.emit(f"Redone: {hist.current_label}", "info")
