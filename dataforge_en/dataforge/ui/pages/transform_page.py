"""Transform page — 30+ operations with a generated form and full history."""
...
class TransformPage(BasePage):
    title = "Transform"
    subtitle = ("Filter, clean, string operations, derived columns, encoding, grouping — "
                "every operation is undoable and kept in the history.")
    ...
    def _build(self) -> None:
        ...
        card_ops = Card("Operations", f"{len(transform.OPS)} available")
        ...
        self.form_card = Card("Operation settings", "pick an operation on the left")
        ...
        btns = QHBoxLayout()
        self.apply_btn = make_button("Apply", "Primary", self._apply)
        self.apply_btn.setEnabled(False)
        btns.addWidget(self.apply_btn)
        btns.addWidget(make_button("Undo", "Ghost", self.store.undo))
        btns.addWidget(make_button("Redo", "Ghost", self.store.redo))
        btns.addStretch(1)
        self.form_card.add_layout(btns)
        mv.addWidget(self.form_card)

        quick = Card("Quick actions", "the most frequently used")
        qrow1 = QHBoxLayout()
        for label, key, params in (
            ("Drop duplicates", "drop_duplicates", {}),
            ("Drop empty rows", "dropna", {"how": "all"}),
            ("Drop constant columns", "remove_constant", {}),
        ...
        hist = Card("Operation history", "click an entry to go back to that state")
        ...
        rv.setSpacing(10)
        prev = Card("Result", "first 200 rows")
    ...
    def _build_form(self, spec: transform.OpSpec) -> None:
        ...
        for p in spec.params:
            ...
            label = p["label"] + ("" if not p.get("optional") else " (optional)")
            widget: Any
    ...
    def _apply(self) -> None:
        ...
        for p in spec.params:
            ...
            v = params.get(p["name"])
            if v in (None, "", []):
                self.notify(f"Please fill in '{p['label']}'", "warn")
                return
    ...
    def refresh(self) -> None:
        ...
        self.shape_label.setText(
            f"{len(df):,} rows × {df.shape[1]} columns" if not df.empty
            else "No data loaded")
