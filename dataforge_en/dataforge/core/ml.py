...
* **AutoML** — automatic task detection (classification/regression), comparison of
  several models with cross-validation, selection of the best one
* **Clustering** — KMeans (auto-k), DBSCAN, Agglomerative, GaussianMixture
* **Anomaly detection** — IsolationForest, LOF, OneClassSVM, z-score, IQR, EWMA
* **Dimensionality reduction** — PCA, t-SNE, TruncatedSVD
* **Text/log clustering** — TF-IDF + KMeans with top terms per cluster
* **Forecasting** — Holt-Winters with a trend fallback
* **Model persistence** and applying a saved model to new data
"""
...
# ---------------------------------------------------------------------------
# Task detection and preparation
# ---------------------------------------------------------------------------
...
def split_feature_types(X: pd.DataFrame) -> tuple[list[str], list[str], list[str]]:
    """Split features into numeric / categorical / text groups."""
    ...
    for c in X.columns:
        ...
        if kind in (NUMERIC, BOOLEAN):
            ...
            continue  # datetime features are expanded separately
...
def expand_datetime(X: pd.DataFrame) -> pd.DataFrame:
    """Turn datetime columns into numeric features."""
    out = X.copy()
...
def build_preprocessor(X: pd.DataFrame, use_text: bool = True,
    ...
    """Build a ColumnTransformer matched to the feature types."""
    ...
    if txt and use_text:
        # only the first text column — TF-IDF must not explode the feature space
        col = txt[0]
        parts.append((
            f"txt_{col}",
            Pipeline([
    ...
    if not parts:
        raise ValueError("No usable feature was found for modelling")
    return ColumnTransformer(parts, remainder="drop", sparse_threshold=0.0)


class _ToString:
    """Tiny transformer that converts input to strings for TF-IDF."""
...
def model_zoo(task: str, fast: bool = False) -> dict[str, Any]:
    """Models suited to the given task."""
    ...
    if task == CLASSIFICATION:
        ...
        if fast:
            for k in ("SVM (RBF)", "Neural Network (MLP)", "Gradient Boosting", "SGD"):
                zoo.pop(k, None)
        return zoo
    ...
    zoo = {
        ...
        "SVR": SVR(),
        "Neural Network (MLP)": MLPRegressor(hidden_layer_sizes=(128, 64),
                                             max_iter=500, random_state=42),
    }
    if fast:
        for k in ("SVR", "Neural Network (MLP)", "Gradient Boosting", "ElasticNet"):
            zoo.pop(k, None)
    return zoo
...
def _classification_metrics(y_true, y_pred, y_proba=None) -> dict[str, float]:
    ...
    m = {
        ...
        "precision (macro)": float(precision_score(y_true, y_pred, average="macro",
                                                   zero_division=0)),
        "recall (macro)": float(recall_score(y_true, y_pred, average="macro",
                                             zero_division=0)),
...
def run_automl(df: pd.DataFrame, target: str, features: list[str] | None = None,
    ...
    ``progress`` — a callable with the signature ``progress(msg: str, pct: int)``.
    """
    ...
    if len(data) < 20:
        raise ValueError(f"Too little data ({len(data)} rows). At least 20 are required.")
    ...
    if task == CLASSIFICATION:
        ...
        if not rare.empty:
            ...
            notes.append(f"{len(rare)} extremely rare class(es) were dropped")
        if y.nunique() < 2:
            raise ValueError("The target must contain at least 2 distinct classes")
    else:
        y = pd.to_numeric(y, errors="coerce")
        ok = y.notna()
        X, y = X[ok.to_numpy()], y[ok.to_numpy()]

    report("Splitting the data…", 5)
    ...
    for i, (name, est) in enumerate(zoo.items(), 1):
        report(f"Training: {name} ({i}/{total})", int(10 + 70 * i / total))
        ...
        try:
            ...
            if cv and cv > 1 and len(X_tr) >= cv * 5:
                try:
                    ...
                    row["CV mean"] = round(float(np.mean(sc)), 4)
            ...
            rows.append({"model": name, "error": str(exc)[:120]})

    if not fitted:
        raise RuntimeError("No model could be trained. Check the selected features.")

    board = pd.DataFrame(rows)
    key = "F1 (macro)" if task == CLASSIFICATION else "R²"
    ...
    report("Evaluating the best model…", 85)
    ...
    if task == CLASSIFICATION:
        ...
        confusion = pd.DataFrame(cm, index=[f"actual {c}" for c in classes],
    ...
    report("Computing feature importance…", 92)
...
def compute_importance(pipe: Any, X: pd.DataFrame, y: pd.Series,
    ...
    """Feature importance — from the model itself or via permutation."""
    try:
        ...
        if hasattr(model, "feature_importances_") and names is not None:
            ...
            if len(vals) == len(names):
                out = pd.DataFrame({"feature": names, "importance": vals})
                return out.sort_values("importance", ascending=False).head(40).reset_index(drop=True)
        ...
        out = pd.DataFrame({"feature": list(X.columns),
...
def roc_curves(y_true: np.ndarray, y_proba: np.ndarray,
    ...
    """ROC curves per class (fpr, tpr, auc)."""
...
def learning_curve_data(pipe: Any, X: pd.DataFrame, y: pd.Series, task: str,
    ...
    return pd.DataFrame({
        "size": sizes,
        "training": train.mean(axis=1),
        "validation": test.mean(axis=1),
    })
...
def cluster(df: pd.DataFrame, columns: list[str], algo: str = "KMeans",
    ...
    """Partition the data into clusters and score the result."""
    ...
    prof = num.copy()
    prof["cluster"] = labels
    profile = prof.groupby("cluster").mean(numeric_only=True).round(4).reset_index()
...
def auto_k(X: np.ndarray, kmin: int = 2, kmax: int = 8,
    ...
    """Find the optimal number of clusters by silhouette score."""
    ...
    for k in range(max(2, kmin), max(3, kmax) + 1):
        try:
            ...
            rows.append({"k": k, "silhouette": round(s, 4),
...
ANOMALY_METHODS = [
    ...
    "Elliptic Envelope", "Z-score", "IQR", "EWMA (time series)",
]
...
def detect_anomalies(df: pd.DataFrame, columns: list[str],
    ...
    """Find anomalous rows across the selected columns."""
    ...
    if method == "EWMA (time series)":
        ...
        detail = pd.DataFrame({"value": col.to_numpy(), "ewma": mean.to_numpy(),
...
def reduce_dim(df: pd.DataFrame, columns: list[str], method: str = "PCA",
    ...
    """Project onto 2–3 dimensions with PCA / t-SNE / SVD."""
...
def text_cluster(texts: pd.Series, k: int = 6, max_features: int = 5000,
    ...
    """Group text (log messages) into topics with TF-IDF + KMeans."""
    from sklearn.cluster import KMeans
    ...
    if len(s) < k * 2:
        raise ValueError(f"Too little text ({len(s)} rows)")
...
def forecast(series: pd.Series, periods: int = 24,
    ...
    """Forecast a time series (Holt-Winters, with a trend fallback)."""
    s = series.dropna().astype(float)
    if len(s) < 8:
        raise ValueError("Forecasting needs at least 8 points")
    ...
    return pd.DataFrame({
        ...
        "lower": np.asarray(pred, dtype=float) - 1.96 * sigma,
        "upper": np.asarray(pred, dtype=float) + 1.96 * sigma,
    })
