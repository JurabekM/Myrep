...
Files created (all inside ``samples/``):

* ``web_access.log``   — Apache combined access log (with a bot-scan episode)
* ``app.log``          — application log in Python logging format (with an error burst)
* ``syslog.log``       — RFC3164 syslog
* ``iot_sensors.csv``  — sensor telemetry with a fault label → for ML
* ``network_flows.jsonl`` — network flows (JSON Lines)
* ``sales.xlsx``       — a business table (for regression)
"""
...
def gen_web_access(n: int = 12_000) -> Path:
    ...
    for i in range(n):
        ...
        ref = rnd.choice(["-", "https://google.com/", "https://example.com/",
                          "https://t.me/", "-"])
        lines.append(
        ...
        # day 3: a bot scanning episode (anomaly)
...
def gen_app_log(n: int = 8_000) -> Path:
    ...
    templates = [
        ("INFO", "User {uid} signed in, session {sid}"),
        ("INFO", "Request completed: {method} {path} — {ms}ms"),
        ("DEBUG", "Cache hit key=user:{uid} ttl={ms}s"),
        ("INFO", "Order created id={oid} amount={amt}"),
        ("WARN", "Slow request: {path} took {ms}ms, over the threshold"),
        ("WARN", "Connection pool is {ms}% full"),
        ("ERROR", "Database connection lost: timeout after {ms}ms host=db-{uid}"),
        ("ERROR", "Payment declined order={oid} code=INSUFFICIENT_FUNDS"),
        ("CRITICAL", "Service not responding: worker-{uid} is being restarted"),
    ]
    ...
    for i in range(n):
        ...
        # an error burst between 12:00 and 12:20 on day 2
        ...
        msg = tmpl.format(uid=rnd.randint(1, 900), sid=f"{rnd.getrandbits(48):012x}",
                          ...
                          amt=round(rnd.uniform(10.0, 4_000.0), 2))
...
def gen_iot(n: int = 20_000) -> Path:
    """Sensor telemetry with a 'fault' target for ML classification."""
    ...
    zones = {d: z for d, z in zip(devices, rng.choice(
        ["warehouse", "line-1", "line-2", "office"], len(devices)))}
    ...
    df = pd.DataFrame({
        "timestamp": ts,
        "device_id": dev,
        "zone": [zones[d] for d in dev],
        "temperature_c": temp.round(2),
        "humidity_pct": np.round(hum, 2),
        "pressure_hpa": press.round(2),
        "voltage_v": volt.round(3),
        "vibration_g": vib.round(3),
        "rssi_dbm": rssi.round(1),
        "fault": fault,
    })
    df["state"] = np.where(df["fault"] == 1, "error",
                           np.where(df["temperature_c"] > 30, "warning", "normal"))
...
def gen_sales(n: int = 4_000) -> Path:
    ...
    cats = ["Electronics", "Groceries", "Apparel", "Construction", "Appliances"]
    ...
    for i in range(n):
        ...
        unit = float(abs(rng.normal({"Electronics": 350.0, "Groceries": 4.5,
                                     "Apparel": 26.0, "Construction": 18.0,
                                     "Appliances": 210.0}[cat], 6.0)))
        ...
        rows.append({
            "date": d, "region": region, "category": cat, "channel": ch,
            "quantity": qty, "unit_price": round(unit, 2), "discount": disc,
            "customer_age": int(np.clip(rng.normal(37, 12), 18, 78)),
            "repeat_customer": int(rng.random() < 0.42),
