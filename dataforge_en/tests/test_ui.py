"""GUI layer tests (run offscreen — no display required)."""
...
def df() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "msg": ["event " + str(i) for i in range(n)],
    })
...
class TestDataStore:
    ...
    def test_run_op_invalid_returns_false(self, app, df):
        s = DataStore()
        s.add("d", df)
        assert s.run_op("filter_query", expr="does_not_exist > 1") is False
        assert len(s.df()) == len(df)   # the data was left intact
    ...
    def test_set_cell_bad_column(self, app, df):
        s = DataStore()
        s.add("d", df)
        assert s.set_cell(0, "nope", 1) is False
    ...
    def test_add_row_and_column(self, app, df):
        ...
        s.add_column("extra", 0)
        assert "extra" in s.df().columns
    ...
    def test_history_labels(self, app, df):
        ...
        assert len(labels) == 2 and "First" in labels[1]
