"""Core layer tests: loading, log parsing, profiling, transforms, ML, charts."""
...
class TestLogParse:
    ...
    def test_detect_pylog(self):
        lines = [f"2024-05-01 10:00:0{i},123 - api.auth - ERROR - failure {i}"
                 for i in range(9)]
    ...
    def test_detect_raw_fallback(self):
        lines = ["just some text", "another line", "no structure at all"]
    ...
    def test_parse_empty(self):
        ...
        assert df.empty and fmt.name == "empty"
    ...
    def test_mine_templates_groups(self):
        msgs = pd.Series([f"Request {i} completed in {i * 10}ms" for i in range(50)]
                         + ["Service started"] * 3)
    ...
    def test_rare_templates(self):
        msgs = pd.Series(["a routine event"] * 1000 + ["a very rare event"])
        _, table = logparse.mine_templates(msgs)
        rare = logparse.rare_templates(table, threshold=0.01)
        assert len(rare) == 1
        assert "rare" in rare.iloc[0]["template"]

    def test_hex_ids_are_masked(self):
        msgs = pd.Series([f"session {i:012x} opened" for i in range(200)])
        _, table = logparse.mine_templates(msgs)
        assert len(table) == 1
        assert "<HEX>" in table.iloc[0]["template"]
    ...
    def test_enrich_extracts_ip(self):
        df = pd.DataFrame({"message": ["connection from 192.168.1.10", "no error"]})
...
class TestIngest:
    ...
    def test_detect_encoding_utf8(self):
        assert ingest.detect_encoding("hello world".encode()) == "utf-8"
    ...
    def test_missing_file_raises(self):
        with pytest.raises(FileNotFoundError):
            ingest.load_file("__does_not_exist__.csv")
...
class TestProfile:
    ...
    def test_issues_detect_constant(self):
        ...
        assert any("Constant" in i["title"] for i in issues)
    ...
    def test_top_correlations(self):
        ...
        assert len(top) == 1 and top.iloc[0]["strength"] == "very strong"
...
class TestTransform:
    ...
    def test_fillna_median(self):
        ...
        out = transform.apply_op(df, "fillna", columns=["a"], method="median")
    ...
    def test_rename(self, sample_df):
        out = transform.apply_op(sample_df, "rename", column="temp", new_name="temperature")
        assert "temperature" in out.columns and "temp" not in out.columns

    def test_astype_variants(self, sample_df):
        out = transform.apply_op(sample_df, "astype", column="fault", dtype="text")
        assert out["fault"].dtype == object
        back = transform.apply_op(out, "astype", column="fault", dtype="integer")
    ...
    def test_str_op(self, sample_df):
        out = transform.apply_op(sample_df, "str_op", column="msg", action="uppercase")
    ...
    def test_datetime_parts(self, sample_df):
        out = transform.apply_op(sample_df, "datetime_parts", column="ts",
                                 parts=["year", "hour"])
        assert "ts_year" in out.columns and out["ts_year"].iloc[0] == 2024
...
class TestML:
    ...
    def test_automl_bad_target(self, sample_df):
        with pytest.raises(ValueError):
            ml.run_automl(sample_df, target="no_such_column")
    ...
    def test_anomaly_ewma(self, sample_df):
        res = ml.detect_anomalies(sample_df, ["temp"], method="EWMA (time series)")
    ...
    def test_text_cluster(self):
        msgs = pd.Series(["connection error timeout"] * 40
                         + ["user signed in session"] * 40
                         + ["payment completed successfully"] * 40)
...
class TestCharting:
    ...
    def test_unknown_chart_raises(self, sample_df):
        with pytest.raises(ValueError):
            charting.build_chart(sample_df, "no_such_chart", x="temp")
    ...
    def test_ml_figures(self, sample_df):
        cm = pd.DataFrame([[10, 2], [3, 15]], index=["actual 0", "actual 1"],
                          columns=["predicted 0", "predicted 1"])
        assert charting.confusion_figure(cm) is not None
        imp = pd.DataFrame({"feature": ["a", "b"], "importance": [0.7, 0.3]})
    ...
    def test_chart_types_registry(self):
        ...
        assert "Basic" in charting.chart_groups()
...
class TestReport:
    ...
    def test_report_with_figures(self, sample_df):
        ...
        html = report.build_report(sample_df, figures=[(fig, "Temperature")])
...
class TestProject:
    def test_save_load_roundtrip(self, tmp_path, sample_df):
        p = project.save_project(tmp_path / "proj", {"a": sample_df.head(50),
                                                     "b": sample_df.head(10)},
                                 active="a")
...
class TestIntegration:
    def test_full_pipeline(self, tmp_path, apache_lines):
        """Log → load → transform → profile → ML → report."""
        p = tmp_path / "access.log"
        ...
        df = transform.apply_op(df, "datetime_parts", column="ts", parts=["hour"])
        assert "ts_hour" in df.columns

        prof = profile.profile_dataframe(df)
        assert prof.rows == 180

        # paths are masked entirely as <PATH>, so mine templates on the agent column
        tids, table = logparse.mine_templates(df["agent"].astype(str))
        assert len(table) >= 2
        assert int(table["count"].sum()) == 180

        df["failed"] = (df["status"] >= 400).astype(int)
        ml_res = ml.run_automl(df[["method", "path", "bytes", "ts_hour", "failed"]],
                               target="failed", fast=True, cv=0)
        assert ml_res.metrics["accuracy"] > 0.5

        html = report.build_report(df, profile=prof, ml_result=ml_res,
                                   dataset_name="access")
        assert "Machine Learning results" in html
